...
Ref<SiOPMWaveTable> SiONDriver::set_wave_table(int p_index, Vector<double> p_table) {
	...
	wave_data.resize_initialized(1 << bits);
...
void SiONDriver::_update_volume() {
	...
	double db_volume = Math::linear_to_db(_master_volume * _fader_volume);
...
void SiONDriver::_prepare_render(const Variant &p_data, int p_buffer_size, int p_buffer_channel_num, bool p_reset_effector) {
	...
	_render_buffer.resize_initialized(p_buffer_size);
...
bool SiONDriver::_rendering() {
	...
	if (_render_buffer.size() < (_render_buffer_index + buffer_extension)) {
		_render_buffer.resize_initialized(_render_buffer_index + buffer_extension);
	}
...
void SiONDriver::_streaming() {
	...
	int start_time = OS::get_singleton()->get_ticks_msec();
	...
	const int frame_time = OS::get_singleton()->get_ticks_msec() - start_time;
...
Ref<SiONData> SiONDriver::compile(String p_mml) {
	...
	_performance_stats.compiling_time = OS::get_singleton()->get_ticks_msec() - start_time;
	_mml_string = "";
...
PackedFloat64Array SiONDriver::render(const Variant &p_data, int p_buffer_size, int p_buffer_channel_num, bool p_reset_effector) {
	...
	_performance_stats.rendering_time = OS::get_singleton()->get_ticks_msec() - start_time;
...
void SiONDriver::stream(bool p_reset_effector) {
	...
	stop();
	_prepare_stream(Variant(), p_reset_effector);
}
...
void SiONDriver::_fade_callback(double p_value) {
	if (!_has_initialized || _cleaning_up) {
		return;
	}
...
void SiONDriver::_set_processing_immediate() {
	...
	_performance_stats.frame_timestamp = OS::get_singleton()->get_ticks_msec();
}
...
void SiONDriver::_process_frame_queue() {
	...
	switch (_current_job_type) {
		...
		case JobType::RENDER: {
			...
			int rendering_time = OS::get_singleton()->get_ticks_msec() - start_time;
			while (rendering_time <= _queue_interval) {
				...
				rendering_time = OS::get_singleton()->get_ticks_msec() - start_time;
			}
...
void SiONDriver::_process_frame_immediate() {
	...
	int t = OS::get_singleton()->get_ticks_msec();
...
void SiONDriver::_notification(int p_what) {
	switch (p_what) {
		case NOTIFICATION_ENTER_TREE: {
			...
			ERR_FAIL_COND_MSG((_buffer_length != 2048 && _buffer_length != 4096 && _buffer_length != 8192), "SiONDriver: Buffer length can only be 2048, 4096, or 8192.");
			ERR_FAIL_COND_MSG((_channel_num != 1 && _channel_num != 2), "SiONDriver: Channel number can only be 1 (mono) or 2 (stereo).");
			ERR_FAIL_COND_MSG((_sample_rate != 44100), "SiONDriver: Sampling rate can only be 44100.");
			...
			// Main sound.
			{
				...
				_audio_stream->set_mix_rate(_sample_rate);
				_audio_stream->set_buffer_length((double)_buffer_length / _sample_rate);	
		...
		case NOTIFICATION_EXIT_TREE: {
			...
			_background_fader = nullptr;
			_background_voice = nullptr;
			_timer_sequence = nullptr;
			if (_performance_stats.processing_time_data != nullptr) {
				memdelete(_performance_stats.processing_time_data);
				_performance_stats.processing_time_data = nullptr;
			}

			if (_timer_sequence != nullptr) {
