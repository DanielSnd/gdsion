...
#include "effects/si_effect_autopan.h"
#include "effects/si_effect_compressor.h"
#include "effects/si_effect_distortion.h"
#include "effects/si_effect_downsampler.h"
#include "effects/si_effect_equalizer.h"
#include "effects/si_effect_speaker_simulator.h"
#include "effects/si_effect_stereo_chorus.h"
#include "effects/si_effect_stereo_delay.h"
#include "effects/si_effect_stereo_expander.h"
#include "effects/si_effect_stereo_reverb.h"
#include "effects/si_effect_wave_shaper.h"
#include "filters/si_controllable_filter_base.h"
#include "filters/si_controllable_filter_high_pass.h"
#include "filters/si_controllable_filter_low_pass.h"
#include "filters/si_filter_all_pass.h"
#include "filters/si_filter_band_pass.h"
#include "filters/si_filter_high_boost.h"
#include "filters/si_filter_high_pass.h"
#include "filters/si_filter_low_boost.h"
#include "filters/si_filter_low_pass.h"
#include "filters/si_filter_notch.h"
#include "filters/si_filter_peak.h"
#include "filters/si_filter_vowel.h"
...
SiEffectStream *SiEffector::_get_global_stream(int p_slot) {
	...
	if (_global_effects.get(p_slot) == nullptr) {
	...
	return _global_effects.get(p_slot);
}
...
Ref<SiEffectBase> SiEffector::get_effect_instance(const String &p_name) {
	...
	List<Ref<SiEffectBase>> instances = _effect_instances.get(p_name);

	// Check if we have free instances to reuse first.
	for (int i = 0; i < instances.size(); i++) {
		if (instances.get(i)->is_free()) {
			Ref<SiEffectBase> effect = instances.get(i);
...
Array SiEffector::get_slot_effects(int p_slot) const {
	ERR_FAIL_INDEX_V_MSG(p_slot, SiOPMSoundChip::STREAM_SEND_SIZE, Array(), "SiEffector: Invalid effect slot index.");

	if (!_global_effects.get(p_slot)) {
		return Array();
	}

	List<Ref<SiEffectBase>> chained_effects = _global_effects.get(p_slot)->get_chain();
...
void SiEffector::set_slot_effects(int p_slot, const Array &p_effects) {
	ERR_FAIL_INDEX_MSG(p_slot, SiOPMSoundChip::STREAM_SEND_SIZE, "SiEffector: Invalid effect slot index.");
...
void SiEffector::clear_slot_effects(int p_slot) {
	...
	if (p_slot == 0) {
		...
		if (_global_effects.get(p_slot) != nullptr) {
			_global_effects.get(p_slot)->free();
			_free_effect_streams.push_back(_global_effects.get(p_slot));
...
void SiEffector::prepare_process() {
	...
	for (int i = 1; i < SiOPMSoundChip::STREAM_SEND_SIZE; i++) {
		...
		if (_global_effects.get(i) != nullptr) {
			int channel_count = _global_effects.get(i)->prepare_process();
			if (channel_count > 0) {
				_sound_chip->set_stream_slot(i, _global_effects.get(i)->get_stream());
				_global_effect_count++;
			}
		}
	}
...
void SiEffector::begin_process() {
	...
	for (int i = 1; i < SiOPMSoundChip::STREAM_SEND_SIZE; i++) {
		if (_global_effects.get(i) != nullptr) {
			_global_effects.get(i)->get_stream()->clear();
		}
	}
}
...
void SiEffector::end_process() {
	...
	for (int i = 1; i < SiOPMSoundChip::STREAM_SEND_SIZE; i++) {
		if (_global_effects.get(i) != nullptr) {
			SiEffectStream *effect = _global_effects.get(i);
...
void SiEffector::initialize() {
	...
	for (int i = 1; i < SiOPMSoundChip::STREAM_SEND_SIZE; i++) {
		if (_global_effects.get(i) != nullptr) {
			_global_effects.get(i)->free();
			_free_effect_streams.push_back(_global_effects.get(i));
...
SiEffector::SiEffector(SiOPMSoundChip *p_chip) {
	...
	_global_effects.resize_initialized(SiOPMSoundChip::STREAM_SEND_SIZE);
	_global_effects.write[0] = _master_effect;
...
SiEffector::~SiEffector() {
	...
	if (_has_initialized) {
		// Clean up free effect streams
		if (_free_effect_streams.size() > 0) {
		...
		// Clean up local effects
		if (_local_effects.size() > 0) {
			for (SiEffectStream *effect : _local_effects) {
				if (effect != nullptr) {
					memdelete(effect);
				}
    ...
    // Clean up global effects - BUT skip index 0 (_master_effect)
    if (_global_effects.size() > 0) {
        for (int i = 1; i < _global_effects.size(); i++) { // Start from 1, not 0!
        ...
        // Now safely delete _master_effect separately
        if (_master_effect != nullptr) {
            memdelete(_master_effect);
