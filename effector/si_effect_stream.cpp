...
int SiEffectStream::prepare_process() {
	...
	_stream->set_channel_count(_chain.get(0)->prepare_process());
	for (int i = 1; i < _chain.size(); i++) {
		_chain.get(i)->prepare_process();
	}
...
int SiEffectStream::process(int p_start_idx, int p_length, bool p_write_in_stream) {
	...
	for (int i = 0; i < _chain.size(); i++) {
		channel_count = _chain.get(i)->process(channel_count, buffer, p_start_idx, p_length);
	}
	...
	if (p_write_in_stream) {
		if (_has_effect_send) {
			for (int i = 0; i < SiOPMSoundChip::STREAM_SEND_SIZE; i++) {
				if (_volumes.get(i) > 0) {
					SiOPMStream *stream = _output_streams.get(i);
					if (!stream) {
						stream = _sound_chip->get_stream_slot(i);
					}
					if (stream) {
						stream->write_from_vector(buffer, p_start_idx, p_start_idx, p_length, _volumes.get(i), _pan, 2);
					}
				}
			}
		} else {
			SiOPMStream *stream = _output_streams.get(0);
			if (!stream) {
				stream = _sound_chip->get_output_stream();
			}
			stream->write_from_vector(buffer, p_start_idx, p_start_idx, p_length, _volumes.get(0), _pan, 2);
		}
	}
...
void SiEffectStream::parse_mml(int p_slot, String p_mml, String p_postfix) {
	...
	int argc = 0;
	Vector<double> args;
	args.resize_initialized(max_argc);
...
#define CONSUME_ARG(m_index)                                     \
	...
	Array matches = re_mml->search_all(p_mml);
	for (int i = 0; i < matches.size(); i++) {
		Ref<RegExMatch> res = matches.get(i);
...
SiEffectStream::SiEffectStream(SiOPMSoundChip *p_chip, SiOPMStream *p_stream) {
	...
	_volumes.resize_initialized(SiOPMSoundChip::STREAM_SEND_SIZE);
	_output_streams.resize_initialized(SiOPMSoundChip::STREAM_SEND_SIZE);
	for (int i = 0; i < SiOPMSoundChip::STREAM_SEND_SIZE; i++) {
		_output_streams.write[i] = nullptr;
	}
}
