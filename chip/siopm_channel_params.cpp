...
Ref<SiOPMOperatorParams> SiOPMChannelParams::get_operator_params(int p_index) {
	...
	return operator_params.get(p_index);
}
...
double SiOPMChannelParams::get_master_volume(int p_index) const {
	...
	return master_volumes.get(p_index);
}
...
void SiOPMChannelParams::set_by_opm_register(int p_channel, int p_address, int p_data) {
	if (p_address < 0x20) { // Module parameter
		switch (p_address) {
			case 15: { // NOIZE:7 FREQ:4-0 for channel#7
				if (p_channel == 7 && (p_data & 128)) {
					operator_params.get(3)->pulse_generator_type = SiONPulseGeneratorType::PULSE_NOISE_PULSE;
					operator_params.get(3)->pitch_table_type = SiONPitchTableType::PITCH_TABLE_OPM_NOISE;
					operator_params.get(3)->fixed_pitch = ((p_data & 31) << 6) + 2048;
				}
			} break;
		...
		if (p_address < 0x40) { // Channel parameter
			...
			Ref<SiOPMOperatorParams> op_params = operator_params.get(op_index);
...
void SiOPMChannelParams::initialize() {
	...
	for (int i = 0; i < MAX_OPERATORS; i++) {
		operator_params.get(i)->initialize();
	}
...
void SiOPMChannelParams::copy_from(const Ref<SiOPMChannelParams> &p_params) {
	...
	for (int i = 0; i < MAX_OPERATORS; i++) {
		operator_params.get(i)->copy_from(p_params->operator_params.get(i));
	}
...
SiOPMChannelParams::SiOPMChannelParams() {
	...
	master_volumes.resize_initialized(SiOPMSoundChip::STREAM_SEND_SIZE);
