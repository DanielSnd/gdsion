...
#include "../wave/siopm_wave_pcm_data.h"
#include "../wave/siopm_wave_pcm_table.h"
...
void SiOPMChannelPCM::_no_process_pcm(int p_length) {
...
void SiOPMChannelPCM::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("_no_process_pcm", "length"), &SiOPMChannelPCM::_no_process_pcm);
}

SiOPMChannelPCM::SiOPMChannelPCM(SiOPMSoundChip *p_chip) : SiOPMChannelBase(p_chip) {
	_operator = memnew(SiOPMOperator(p_chip));
	_process_function = Callable(this, "_no_process_pcm");
