...
#include "channels/siopm_channel_manager.h"
...
SiOPMSoundChip::SiOPMSoundChip() {
	...
	stream_slot.resize_initialized(STREAM_SEND_SIZE);
	stream_slot.fill(nullptr);

	zero_buffer = memnew(SinglyLinkedList<int>(1, 0, true));
	_pipe_buffers.resize_initialized(PIPE_SIZE);
