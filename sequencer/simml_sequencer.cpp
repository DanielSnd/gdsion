...
#include "base/mml_executor.h"
#include "base/mml_executor_connector.h"
#include "base/mml_parser.h"
#include "base/mml_parser_settings.h"
#include "base/mml_sequence.h"
#include "base/mml_sequence_group.h"
#include "base/mml_system_command.h"
...
String SiMMLSequencer::_on_before_compile(String p_mml) {
	...
	for (int i = 0; i < matches.size(); i++) {
		...
		for (int j = 0; j < mid_matches.size(); j++) {
			...
			if (mid_res->get_string(0).is_empty()) {
	...
	for (int i = matches.size() - 1; i >= 0; i--) {
		...
		expanded_mml = expanded_mml.substr(0, res->get_start(0)) + replacement + expanded_mml.substr(res->get_end(0) + 1);
	}
...
String SiMMLSequencer::_expand_macro(String p_macro, uint32_t p_macro_flags) {
	...
	for (int i = matches.size() - 1; i >= 0; i--) {
		...
		int flag = 1 << index;
		ERR_FAIL_COND_V_MSG(expanded_macro_flags & flag, p_macro, vformat("SiMMLSequencer: Failed to expand a macro due to a circular reference, '%s'.", res->get_string(0)));
		...
		// Take the rest of the string (around the match) and insert the replaced substring.
		const String prefix = expanded_macro.substr(0, res->get_start(0));
		const String suffix = expanded_macro.substr(res->get_end(0));
...
#define GET_EV_PARAMS(m_count)                                           \
	...
	ev_params.resize_initialized(MAX_PARAM_COUNT);                            \
...
SiMMLSequencer::SiMMLSequencer(SiOPMSoundChip *p_chip) :
	...
	_macro_strings.resize_initialized(MACRO_SIZE);
