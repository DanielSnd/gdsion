...
MMLEvent *MMLSequencer::_default_on_internal_call(MMLEvent *p_event) {
	...
	if (callback_idx >= 0 && callback_idx < callbacks.size()) {
		Callable cb = callbacks.get(callback_idx);
		if (cb.is_valid()) {
...
MMLSequencer::MMLSequencer() {
	...
	_event_handlers.resize_initialized(MMLEvent::COMMAND_MAX);
	_event_global_flags.resize_initialized(MMLEvent::COMMAND_MAX);
