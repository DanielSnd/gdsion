...
int MMLParser::_register_system_event_string(String p_event) {
	if (_system_event_strings.size() <= _system_event_index) {
		_system_event_strings.resize_initialized(_system_event_strings.size() * 2);
	}
...
int MMLParser::_register_sequence_mml_strings(String p_mml) {
	if (_sequence_mml_strings.size() <= _sequence_mml_index) {
		_sequence_mml_strings.resize_initialized(_sequence_mml_strings.size() * 2);
	}
...
void MMLParser::prepare_parse(MMLParserSettings *p_settings, String p_mml) {
	...
	_mml_string = p_mml;
	_parsing_time = OS::get_singleton()->get_ticks_msec();
...
MMLEvent *MMLParser::parse(int p_interrupt) {
	...
	for (int i = 0; i < matches.size(); i++) {
		...
		_mml_regex_last_index = res->get_end(0) + 1;
	...
	_parsing_time = OS::get_singleton()->get_ticks_msec() - _parsing_time;
...
void MMLParser::_op_repeat_break() {
	...
	_last_event->set_jump(_repeat_stack.get(0));
}
...
bool MMLParser::_op_end_sequence() {
	...
	return _interrupt_interval < (OS::get_singleton()->get_ticks_msec() - _start_time);
}
...
MMLParser::MMLParser() {
	...
	_system_event_strings.resize_initialized(32);
	_sequence_mml_strings.resize_initialized(32);

	_key_signature_custom.resize_initialized(7);
