...
Ref<SiOPMWaveTable> SiMMLData::set_wave_table(int p_index, Vector<double> *p_data) {
	...
	log_table.resize_initialized(p_data->size());
...
SiMMLData::SiMMLData() {
	_envelope_tables.resize_initialized(SiMMLRefTable::ENVELOPE_TABLE_MAX);
	_wave_tables.resize_initialized(SiOPMRefTable::WAVE_TABLE_MAX);
	_sampler_tables.resize_initialized(SiOPMRefTable::SAMPLER_TABLE_MAX);

	_fm_voices.resize_initialized(SiMMLRefTable::VOICE_MAX);
	_pcm_voices.resize_initialized(SiOPMRefTable::PCM_DATA_MAX);
